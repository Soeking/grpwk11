...
int loopBm(char *text, const char *outText, const int *used, str s, int l, int index, int strId) {
    while (true) {
        index = bm(text, s.s, s.table, index, l, s.len);
        if (index < 0) break;
        if ((s.s[0] == 'c' && outText[index - 1] == 'd' && used[index - 1] != strId) ||
            (s.s[s.len - 1] == 'd' && outText[index + s.len] == 'c' && (used[index + s.len] != strId)))
            index += s.len;
        else break;
    }
    return index;
...
void procStr(char *text, char *outText, int *used, int i, str *strs, int l, int size) {
    int index = loopBm(text, outText, used, strs[i], l, strs[i].len + strs[i].id, i);
    while (true) {
        if (index < 0) break;
        if (used[index] >= 0 && used[index + strs[i].len - 1] >= 0) break;
        index = loopBm(text, outText, used, strs[i], l, index + strs[i].len, i);
    }
    if (index < 0) {
        int first = loopBm(text, outText, used, strs[i], l, strs[i].len + strs[i].id, i);
        if (first < 0) first = loopBm(text, outText, used, strs[i], l, strs[i].len - 1, i); //多分あった方がいい(精度落ちたけど)
